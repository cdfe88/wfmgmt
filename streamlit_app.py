...
def workload_ini():
    ...
    wload['Open'] = pd.to_datetime(wload['Open'], format='%H:%M').dt.time
    wload['Close'] = pd.to_datetime(wload['Close'], format='%H:%M').dt.time
    wload['Sat Open'] = pd.to_datetime(wload['Sat Open'], format='%H:%M').dt.time
    wload['Sat Close'] = pd.to_datetime(wload['Sat Close'], format='%H:%M').dt.time
...
def agg_hc(schedule):
    ...
    for _,r in schedule.iterrows():
        ...
        for h in hrs:
            rows.append({'Weekday':r['weekday'],'Hour':h.time(),'Headcount':r['hc']})
    ...
    with colb:
        ...
        with tab3:
            ...
            with st.expander('Suggested Roster per Shift and Business Day', expanded=False):
                st.dataframe(dafr)
            st.write('Suggested Roster v. Calculated Demand')
