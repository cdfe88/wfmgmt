...
def agg_hc(schedule):
    ...
    for _,r in schedule.iterrows():
        hrs= pd.date_range(start=pd.Timestamp.combine(pd.Timestamp.now().date(), r['shift start']),end=pd.Timestamp.combine(pd.Timestamp.now().date(), r['shift end']),freq='h',closed='left')
        for h in hrs:
